...
/*  End iteration loop */
  ...
  fabs(error - resid_ref) / resid_ref < 1E-5?((void )0) : __assert_fail("fabs(error-resid_ref)/resid_ref < 1E-5","jacobi.c",225,__PRETTY_FUNCTION__);
}
...
#pragma omp parallel for private(xx,yy,temp,j,i) reduction(+:error)
  ...
  fabs(error - error_ref) / error_ref < 1E-5?((void )0) : __assert_fail("fabs(error-error_ref)/error_ref < 1E-5","jacobi.c",255,__PRETTY_FUNCTION__);
}
